...
config =
  ...
  roles_list: process.env.HUBOT_AUTH_ROLES
...
module.exports = (robot) ->
  ...
  if config.roles_list?
    for role in config.roles_list.split ';'
      [full, role_name, user_ids] = role.match /(\w+):(\w+(?:,\w+)*)/
  ...
  robot.respond /auth list (["'\w: -_]+) assigned users?$/i, (msg) ->
    ...
    role = msg.match[1]
    names = robot.auth.usersWithRole(role) if role?
    if names.length > 0
      msg.reply 'The following people have the \'' + role +  '\' role: @' + names.join ', @'
    else
